...
нет, x и y остаются ссылками на один и тот же список, что кстати в питоне можно проверить оператором is, я уверен что создатель джавы навсегда лишился хорошего сна когда узнал про этот оператор в питоне.

Строки хотя и похожи на список являются иммутабельным типом данных, это значит, что саму строку изменить нельзя, можно лишь породить новую, но переменной можно присвоить другое значение, хотя данные при этом не изменятся:
